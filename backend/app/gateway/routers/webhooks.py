...
async def watcher_webhook(payload: dict[str, Any], request: Request, background_tasks: BackgroundTasks):
    ...
    async def ai_callback(event: dict[str, Any], history_entry: dict):
        ...
        prompt = (
            ...
            f"3. Allocate a room, and if the patient needs medication, you MUST use the 'check_pharmacy_inventory' tool to verify stock levels before you use the 'book_pharmacy_prescription' tool. If stock is out or low, send an URGENT Out of Stock Alert via Telegram.\n"
            f"4. Use the 'generate_medical_report' tool to autonomously typeset and generate a PDF Medical Report with the patient's diagnosis and notes.\n"
            f"5. CRITICAL: For EVERY action you successfully book or complete "
            f"(e.g., assigning a doctor, booking a room, generating the PDF, placing a pharmacy order), "
